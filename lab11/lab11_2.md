...
```
## No documentation for 'pallete' in specified packages and libraries:
## you could try '??pallete'
```
...
```r
homerange%>%
  ggplot(aes(x=log10.mass,y=log10.hra, color=class))+geom_point()+scale_color_brewer(palette="BrBG")
```
