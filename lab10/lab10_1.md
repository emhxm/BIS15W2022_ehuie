...
homerange%>%
  ...
  labs(title = "Ecotherms vs. Endotherms",
       x = "Thermoregulation",
       y = "Count") +theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
```
...
```r
homerange%>%
  ggplot(aes(x=log10.mass, y=log10.hra))+geom_point(size=1, color="green")+labs(title = "log10.mass vs. log10.hra",
       x = "log10.mass",
       y = "log10.hra") +theme(plot.title = element_text(size = rel(2), hjust = 0.5))
```
