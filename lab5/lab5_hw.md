...
More good guys are bald (no hair N=37), whereas bad guys are (no hair N=35). 
...
We do not have 16 rows in question #10 because we are considering all cases that would match the height>200 as well as if they match weight>=450. These values are considered independent of one another. 
...
```r
superhero_info%>%
  mutate(hwratio=height/weight)%>%
  select(name, hwratio, height, weight)%>%
  arrange(desc(hwratio))
```

```
## # A tibble: 734 x 4
##    name            hwratio height weight
##    <chr>             <dbl>  <dbl>  <dbl>
##  1 Groot            175.      701      4
