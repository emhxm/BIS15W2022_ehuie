...
```r
amniota <- readr::read_csv(file="data/amniota.csv")%>% clean_names()
```
...
```r
amphibio<-readr::read_csv(file="data/amphibio.csv")%>% clean_names()
```
...
For the amniota dataset, NA's are represented as -999 so it makes sense that the actual NA count would be zero. For the amphibio data set, NAs are actually represented as NA's. 
`amniota`  
...
Mammals do not lay eggs so it makes sense that there is 100% missing data. Not all NA's mean that there is missing data and the values aren't useful, the missing values sometimes have meaning. 
...
The NA value could represent that these species are not that classification. For instance, a mammal would not be classified as aquatic. 
...
```r
amniota <- readr::read_csv(file="data/amniota.csv",na = c("NA", " ", ".", "-999"))%>% clean_names()
```
